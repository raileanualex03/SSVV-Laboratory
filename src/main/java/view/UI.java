...
public class UI {
    ...
    public void run() {
        ...
        // Lab 1 - Take home: Modify the source code such that at least the three functionalities are “called”.
        //   Add a Student
        //   Add a Laboratory assignment
        //   Add a Grade to a Student at a given Laboratory assignment

        // 1. Add a student. For this, we will first search if exists a student
        // with id 00000 and if it does, delete it and then add it

        Student studentToAdd = new Student("00000", "New Student", 935, "newstudent@gmail.com");
        Student student = service.findStudent(studentToAdd.getID());
        if (student == null) {
            service.addStudent(studentToAdd);
        } else {
            service.deleteStudent(studentToAdd.getID());
            service.addStudent(studentToAdd);
        }

        // 2. Add a Laboratory assignment: First check if the assignment with number
        // 100 exists, and if it does, delete it and then add a new one

        Tema labAssignmentToAdd = new Tema("100", "New Lab Assignment", 4, 3);
        Tema tema = service.findTema(labAssignmentToAdd.getID());
        if (tema == null)
        {
            service.addTema(labAssignmentToAdd);
        }
        else {
            service.deleteTema(labAssignmentToAdd.getID());
            service.addTema(labAssignmentToAdd);
        }

        // 3. Add a Grade to the added student with id 00000 at the lab assignment with number 100.

        Nota gradeToAdd = new Nota("00000", studentToAdd.getID(), labAssignmentToAdd.getID(), 10, LocalDate.of(2022, 2, 2));
        Nota grade = service.findNota(gradeToAdd.getID());
        if(grade == null){
            service.addNota(gradeToAdd, "Nice work!");
        }
        else{
            service.deleteNota(gradeToAdd.getID());
            service.addNota(gradeToAdd, "Nice work!");
        }
